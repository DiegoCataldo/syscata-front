...
const ContractTable = ({ contracts, page, rowsPerPage, totalCount, handleChangePage, handleChangeRowsPerPage, onDeleteContract }) => {
    ...
    const handleEdit = (contract_id, den) => {
        let user = { ...currentUser, contract_id, contract_den: den };   
        localStorage.setItem('currentUser', JSON.stringify(user));
        setCurrentUser(user);
        navigate(`/SRRole/${contract_id}`);
    };
            ...
            <TableBody>
                {contracts.map((contract) => (
                    <TableRow key={contract.id}>
                        ...
                        <TableCell>
                            <Button variant="contained" onClick={() => handleEdit(contract.id, contract.DEN )} color="primary">IR</Button>
                        </TableCell>
                    </TableRow>
                ))}
            </TableBody>
